 ...
 *  有遮盖
 */
@property(nonatomic,strong)QSCityPickerView*cityPickerView;
/**
 *  无遮盖 替代键盘
 */
...
-(QSCityPickerView *)cityPickerView{
    if (!_cityPickerView) {
        WS(ws);
        _cityPickerView=[QSCityPickerView shareCityPickerView];
...
-(QSCityPicker *)cityPicker{
    if (!_cityPicker) {
        _cityPicker=[QSCityPicker shareCityPicker];
        WS(ws);
