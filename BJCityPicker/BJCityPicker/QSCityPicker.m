...
@interface QSCityPicker()<UIPickerViewDataSource,UIPickerViewDelegate>
{
...
@end
@implementation QSCityPicker
#pragma mark-----这里修改控件属性-----
-(UIButton *)confirmBtn{
    if (!_confirmBtn) {
        _confirmBtn=[[UIButton alloc] init];
        _confirmBtn.frame=CGRectMake( KDeviceWidth-70,5, 50, 30);
...
#pragma mark----创建-----
static QSCityPicker* instance;
+(QSCityPicker *)shareCityPicker{
    if (!instance) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            instance = [[QSCityPicker alloc] init];
        });
    }
    return instance;
}
...
- (void)pickerDidCancle:(cancelSelected)cancelSelected
{
